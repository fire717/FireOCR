...
class ImgGenerator():
    def __init__(self, 
        ...
        self.exist_char = exist_char
        self.exist_char_count_dict = {}
    ...
    def loadData(self):
        ...
        self.exist_char_count_dict = self.readLabel(self.exist_char)
        print("Load exist_char: ", len(self.exist_char_count_dict), self.exist_char_count_dict)
    ...
    def getCharRandom(self,gen_char_dict):
        # 获取要生成的文本
        #根据已有词表和生成的sentence词表补充
        gen_char_count = {}
        total_count = 0
        for char,char_count in self.exist_char_count_dict.items():
            
            if char in gen_char_dict:
                char_count += gen_char_dict[char]

            add_count = 0
            if char_count<self.min_cont:
                #小于min补齐，或者随机生成100条
                add_count = self.min_cont
            else:
                add_count = self.min_cont//10

            gen_char_count[char] = add_count
            total_count += add_count

        print("getCharRandom: ", gen_char_count, total_count)
        while total_count>100:
            text = []

            this_lenth = random.randint(16,20)
            while len(text)<this_lenth:
                while True:
                    char = random.choice(list(gen_char_count.keys()))
                    if gen_char_count[char]>0:
                        gen_char_count[char] -= 1
                        text.append(char)
                        break
    ...
    def run(self, label_path):
        ...
        ### 2.补充生成
        for i, chars in enumerate(self.getCharRandom(gen_char_dict)):
